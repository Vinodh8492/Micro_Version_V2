...
class AsyncClient(base_client.BaseClient):
    ...
    async def disconnect(self, abort=False, reason=None):
        ...
        if self.state == "connected":
            ...
            await self._trigger_event(
                "disconnect", reason or self.reason.CLIENT_DISCONNECT, run_async=False
            )
    ...
    async def _receive_packet(self, pkt):
        ...
        if pkt.packet_type == packet.MESSAGE:
            ...
            await self.disconnect(abort=True, reason=self.reason.SERVER_DISCONNECT)
                ...
                if run_async:
                    ...
                    try:
                        try:
                            ...
                            if (
                                event == "disconnect" and len(args) == 1
                            ):  # pragma: no branch
            ...
            try:
                ...
                if not isinstance(p.data, (str, bytes)):  # pragma: no cover
                    self.logger.warning(
                        ...
                        "close"
                        if p.type
                        in [aiohttp.WSMsgType.CLOSE, aiohttp.WSMsgType.CLOSING]
                        else str(p.type),
                        str(p.data),
                    )
                    await self.queue.put(None)
