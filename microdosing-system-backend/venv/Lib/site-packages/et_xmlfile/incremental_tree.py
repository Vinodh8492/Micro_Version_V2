        ...
        with _get_writer(file_or_filename, encoding) as (write, declared_encoding):
            ...
            if method == "text":
                ...
                if root_ns_only or minimal_ns_only:
                    ...
                    # were used.
                    new_nsmap = _namespaces(self._root, default_namespace, new_nsmap,)
...
def _make_new_ns_prefix(
    nsmap_scope, global_prefixes, local_nsmap=None, default_namespace=None,
):
    i = len(nsmap_scope)
...
def _find_default_namespace_attr_prefix(
    ...
    return _make_new_ns_prefix(
        nsmap, global_prefixes, local_nsmap, provided_default_namespace,
    )
...
def _namespaces(
    elem, default_namespace=None, nsmap=None,
):
                    ...
                    if prefix is None or prefix in out_nsmap:
                        ...
                        if prefix is None or prefix in out_nsmap:
                            prefix = _make_new_ns_prefix(
                                out_nsmap, global_prefixes, nsmap, default_namespace,
                            )
                    if prefix or is_el:
