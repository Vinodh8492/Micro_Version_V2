...
_GetRevArg = Union[
    str, Iterable[Optional[str]], Iterable[str],
]
...
class RevisionMap:
    ...
    def _detect_cycles(
        ...
        total_space = {
            ...
            rev.revision
            for rev in self._iterate_related_revisions(
                lambda r: r.nextrev, bases, map_=cast(_RevisionMapType, rev_map),
            )
        )
                ...
                for branch_label in revision._orig_branch_labels:
                    if branch_label in map_:
                        ...
                        raise RevisionError(
                            ...
                            "used by revision %s"
                            % (branch_label, revision.revision, map_rev.revision,)
                        )
    ...
    def _topological_sort(
        self, revisions: Collection[Revision], heads: Any,
    ) -> List[str]:
    ...
    def _parse_downgrade_target(
        ...
        if match:
            ...
            if rel_int >= 0:
                ...
                rev = self._walk(
                    symbol, rel_int, branch_label, no_overwalk=assert_relative_length,
                )
                if rev is None:
    ...
    def _collect_downgrade_revisions(
        ...
        if branch_label and len(roots) > 1:
            ...
            ancestors = {
                rev.revision
                for rev in self._get_ancestor_nodes(
                    [self._resolve_branch(branch_label)], include_dependencies=False,
                )
            }
        ...
        downgrade_revisions = set(
            self._get_descendant_nodes(
                roots, include_dependencies=True, omit_immediate_dependencies=False,
            )
        )
