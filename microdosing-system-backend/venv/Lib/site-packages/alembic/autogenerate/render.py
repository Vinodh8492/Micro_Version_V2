...
def _render_gen_name(
    autogen_context: AutogenContext, name: sqla_compat._ConstraintName,
...
def _render_cmd_body(
    op_container: ops.OpContainer, autogen_context: AutogenContext,
) -> str:
    buf = StringIO()
...
def _uq_constraint(
    constraint: UniqueConstraint, autogen_context: AutogenContext, alter: bool,
) -> str:
...
def _repr_type(
    type_: TypeEngine, autogen_context: AutogenContext, _skip_variants: bool = False,
) -> str:
...
def _render_ARRAY_type(type_: ARRAY, autogen_context: AutogenContext) -> str:
    return cast(
        str, _render_type_w_subtype(type_, autogen_context, "item_type", r"(.+?\()"),
    )
...
def _fk_colspec(
    fk: ForeignKey, metadata_schema: Optional[str], namespace_metadata: MetaData,
) -> str:
