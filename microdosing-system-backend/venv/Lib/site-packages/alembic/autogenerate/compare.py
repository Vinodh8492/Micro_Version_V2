    ...
    if conn_table is not None:
        ...
        try:
            ...
            conn_indexes = [  # type:ignore[assignment]
                ...
                if autogen_context.run_name_filters(
                    ix["name"], "index", {"table_name": tname, "schema_name": schema},
                )
            ]
    ...
    def obj_added(obj: _constraint_sig):
        if is_index_sig(obj):
            if autogen_context.run_object_filters(
                ...
                log.info(
                    "Detected added index '%r' on '%s'", obj.name, obj.column_names,
                )
        elif is_uq_sig(obj):
    ...
    def obj_removed(obj: _constraint_sig):
        if is_index_sig(obj):
            ...
            if autogen_context.run_object_filters(
                ...
                log.info(
                    "Detected removed unique constraint %r on %r", obj.name, tname,
                )
        else:
            assert False

    def obj_changed(
        old: _constraint_sig, new: _constraint_sig, msg: str,
    ):
        if is_index_sig(old):
...
def _compare_nullable(
    ...
    if conn_col_nullable is not metadata_col_nullable:
        if (
            ...
            log.info(
                "Ignoring nullable change on identity column '%s.%s'", tname, cname,
            )
        else:
...
def _compare_type(
    ...
    if conn_type._type_affinity is sqltypes.NullType:
        log.info(
            "Couldn't determine database type " "for column '%s.%s'", tname, cname,
        )
        return
