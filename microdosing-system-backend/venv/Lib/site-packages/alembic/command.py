...
def init(
    config: Config, directory: str, template: str = "generic", package: bool = False,
) -> None:
    ...
    with util.status(
        f"Creating directory {os.path.abspath(versions)!r}", **config.messaging_opts,
    ):
...
def check(config: "Config") -> None:
    ...
    revision_context = autogen.RevisionContext(config, script_directory, command_args,)
...
def merge(
    ...
    if environment:
        ...
        with EnvironmentContext(
            config, script, fn=nothing, as_sql=False, template_args=template_args,
        ):
            script.run_env()
...
def ensure_version(config: Config, sql: bool = False) -> None:
    ...
    with EnvironmentContext(
        config, script, fn=do_ensure_version, as_sql=sql,
    ):
        script.run_env()
