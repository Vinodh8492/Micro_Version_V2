...
class MySQLProtocol:
    ...
    def make_auth(
        ...
        if is_change_user_request:
            response_payload.append(
                struct.pack(
                    f"<B{len(b_username)}sx", ServerCmd.CHANGE_USER, b_username,
                )
            )
        else:
            filler = "x" * 23
    ...
    def parse_ok(packet: bytes) -> OkPacketType:
        ...
        try:
            ...
            (ok_packet["status_flag"], ok_packet["warning_count"],) = struct.unpack(
                "<HH", packet[0:4]
            )
            packet = packet[4:]
            if packet:
    ...
    def read_text_result(
        ...
        List[Tuple[Optional[bytes], ...]], Optional[EofPacketType],
    ]:
    ...
    def _parse_binary_values(
        self, fields: List[DescriptionType], packet: bytes, charset: str = "utf-8",
    ...
    def read_binary_result(
        ...
        List[Tuple[BinaryProtocolType, ...]], Optional[EofPacketType],
    ]:
        ...
        if parameters or data:
            ...
            for pos, value in enumerate(data):
                ...
                if pos in long_data_used:
                    ...
                    (packed, field_type, _flags,) = self.prepare_binary_integer(value)
