...
class MySQLConnection(MySQLConnectionAbstract):
    ...
    def _do_handshake(self) -> None:
        ...
        if not handshake["capabilities"] & ClientFlag.SSL:
            ...
            if self._ssl.get("verify_cert"):
                raise InterfaceError(
                    "SSL is required but the server doesn't support it", errno=2026,
                )
    ...
    def _open_connection(self) -> None:
        ...
        try:
            ...
            if isinstance(err, (ReadTimeoutError, WriteTimeoutError)):
                raise ConnectionTimeoutError(errno=err.errno, msg=err.msg,) from err
            raise err
        finally:
    ...
    def get_row(
        ...
        **kwargs: Any,
        ...
        (rows, eof) = self.get_rows(
            count=1, binary=binary, columns=columns, raw=raw, read_timeout=read_timeout,
        )
        if rows:
            return (rows[0], eof)
        return (None, eof)
        ...
        if self._client_flags & ClientFlag.CLIENT_QUERY_ATTRIBUTES:
            ...
            for pos, attr_tuple in enumerate(self._query_attrs.items()):
                ...
                if isinstance(value, int):
                    ...
                    (packed, field_type,) = self._protocol.prepare_binary_timestamp(
                        value
                    )
                    values.append(packed)
    ...
    @handle_read_write_timeout()
    def cmd_query_iter(
        self, statements: StrOrBytes, **kwargs: Any,
    ...
    def cmd_stmt_prepare(
        self, statement: bytes, **kwargs: Any,
