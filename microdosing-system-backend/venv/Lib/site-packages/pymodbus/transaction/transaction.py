        ...
        while True:
            ...
            if self.sent_buffer:
                if data.startswith(self.sent_buffer):
                    Log.debug(
                        "sync recv skipping (local_echo): {}", self.sent_buffer, ":hex",
                    )
            ...
            databuffer += data
            used_len, pdu = self.framer.processIncomingFrame(
                self.trace_packet(False, databuffer)
            )
        ...
        if pdu:
            ...
            if not self.is_server:
                if pdu.dev_id != self.request_dev_id:
                    Log.warning(
                        f"ERROR: expected id {self.request_dev_id} but got {pdu.dev_id}, IGNORING."
                    )
                elif pdu.transaction_id != self.request_transaction_id:
                    Log.warning(
                        f"ERROR: expected transaction {self.request_transaction_id} but got {pdu.transaction_id}, IGNORING."
                    )
