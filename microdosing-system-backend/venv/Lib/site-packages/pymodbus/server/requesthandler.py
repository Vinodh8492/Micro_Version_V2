...
class ServerRequestHandler(TransactionManager):
    ...
    def __init__(self, owner, trace_packet, trace_pdu, trace_connect):
        ...
        self.running = False
        super().__init__(
            params, self.framer, 0, True, trace_packet, trace_pdu, trace_connect,
        )
    ...
    def callback_data(self, data: bytes, addr: tuple | None = None) -> int:
        ...
        try:
            ...
            response = ExceptionResponse(
                40, exception_code=ExceptionResponse.ILLEGAL_FUNCTION
            )
        ...
        try:
            if self.server.broadcast_enable and not self.last_pdu.dev_id:
                ...
                for dev_id in self.server.context.slaves():
                    response = await self.last_pdu.update_datastore(
                        self.server.context[dev_id]
                    )
            else:
